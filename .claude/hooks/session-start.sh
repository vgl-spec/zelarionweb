...
manifest_hash() {
  {
    # This repo has no root manifest: the packages live in frontend/ and backend/.
    for m in package.json frontend/package.json backend/package.json; do
      [ -f "$m" ] || continue
      if command -v jq >/dev/null 2>&1; then
        jq -S '.scripts // {}' "$m"
      elif command -v node >/dev/null 2>&1; then
        node -e 'const s=require("fs").readFileSync(process.argv[1],"utf8");const j=JSON.parse(s).scripts||{};console.log(JSON.stringify(j,Object.keys(j).sort()))' "$m"
      else
        cat "$m"
      fi
    done
